...
function login(username,nome){
	...
	else window.location.href="/html_EC.html";
}
function initialize(){
...
function creaCarrello(){
	...
	if (carrellot.length>0){
		...
		carrellot.forEach((prodotto) => {
			...
			prodottoElement.innerHTML = `
				...
				<button onclick="rimuovi('${prodotto.nome}')">Rimuovi</button>
			`;
	...
	if (username!=null&&username!='null'){
		window.location.href=window.location.href="/html_EC.html?username="+username;
	}
	else window.location.href="html_EC.html";
	}
	...
	function controlla_bonus(){
		...
		xhr.open('POST', 'recupera_bonus.php', true);
	...
	function aggiorna_bonus(){
		...
		xhr.open('POST', 'aggiorna_bonus.php', true);
		xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
